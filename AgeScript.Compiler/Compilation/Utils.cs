...
namespace AgeScript.Compiler.Compilation
{
    internal static class Utils
    {
        ...
        public static void Clear2(CompilationResult result, int from, int length)
        {
            if (length <= 0)
            {
                return;
            ...
            {
                result.Rules.AddAction($"set-goal {from + i} 0");
            }
        }
            ...
            int from_offset = 0, int to_offset = 0, bool ref_from_offset = false, bool ref_to_offset = false)
        {
            ...
            if (result.Settings.InlineMemCopy)
            {
                ...
                result.Rules.AddAction($"up-jump-direct c: {target}");
